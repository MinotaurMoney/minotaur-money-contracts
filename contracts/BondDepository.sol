...
contract MINOBondDepository is Ownable, ReentrancyGuard {
    ...
    address public immutable MINO; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MINO when receives principle
    ...
    bool public useStableValuation = true;
    ...
    struct Terms {
        ...
        uint minimumPrice; // vs principle value. 5 decimals (15000 = 0.15)
    ...
    function initializeBondTerms( 
        ...
        //console.log("_vestingTerm 2 %s", _vestingTerm);
        terms = Terms ({
    ...
    function setUseStableValuation ( bool _useStableValuation ) external onlyPolicy() {
        useStableValuation = _useStableValuation;
    }
    ...
    function setAdjustment ( 
        ...
        uint256 maxIncrement = terms.controlVariable.mul( 100 ).div( 1000 );
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 MINO ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MINO
         */

        if ( useStableValuation ) {
            ...
            if ( fee != 0 ) { // fee is transferred to dao 
                IERC20( MINO ).safeTransfer( DAO, fee ); 
            }
        } else {
            IERC20( principle ).safeTransferFrom( msg.sender, treasury, _amount );
            ITreasury( treasury ).mintRewards( address(this), payout, false );
            ITreasury( treasury ).mintRewards( DAO, fee, true );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( MINO ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( MINO ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( MINO ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( MINO ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function payoutFor( uint _value ) public view returns ( uint ) {
        return FixedPoint.fraction( _value, bondPrice() ).decode112with18().div( 1e13 );
    }
    ...
    function bondPrice() public view returns ( uint price_ ) {        
        price_ = terms.controlVariable.mul( debtRatio() ).div( 1e4 );
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( principle ) ).div( 100000 );
        } else {
            price_ = bondPrice().mul( 10 ** IERC20( principle ).decimals() ).div( 100000 );
        }
    }


    /**
     *  @notice calculate current ratio of debt to MINO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( MINO ).totalSupply();
    ...
    /**
     *  @notice calculate amount of MINO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MINO) to the DAO
     *  @return bool
     */
